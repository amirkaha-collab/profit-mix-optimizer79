...
def render_mix_builder(df_active, all_funds_list):
    ...
    with st.expander("⚙️ הגדרות מתקדמות", expanded=False):
        ...
        # Manager filter – show ALL managers from full dataset (df_long global)
        # so unchecked managers remain visible and re-selectable
        all_mgrs_full = sorted(df_long["manager"].unique().tolist())
        n_total  = len(all_mgrs_full)
        ...
        for i, mgr in enumerate(all_mgrs_full):
            with mgr_cols[i % 3]:
                # Count funds across full dataset so number doesn't disappear when deselected
...
def render_results_table(rows, baseline, voting_configured):
  ...
  <td style='color:#6B7280;font-size:10px'>—</td>
